    ...
    return !users ? null : (
        <div className={b("content")}>
            <div className={b("block")}>
                {users.map(user => <ConnectedUser id={user} key={user} />)}

                <div>
                    {offset !== 0 && <Link to={`/users/?offset=${offset - count}&count=${count}`}>{`< Prev`}</Link>}
                    &nbsp;&nbsp;
                    {users.length === count && <Link to={`/users/?offset=${offset + count}&count=${count}`}>{`Next >`}</Link>}
                </div>
            </div>
